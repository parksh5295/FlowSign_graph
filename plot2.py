...
def main():
    ...
    if needs_break:
        ...
        y_data1 = break_position + 0.04 * y_range + wave_pattern  # 0.025 -> 0.04로 증가
        
        # Second wavy line (inner, lower) - parallel to first line
        y_data2 = break_position - 0.001 * y_range + wave_pattern  # 0.0025 -> -0.01로 감소 (간격 확대)
        ...
        ax.plot(x_data, y_data1, 'k-', linewidth=5, clip_on=False, zorder=15)  # linewidth=7 -> 10으로 증가
        ax.plot(x_data, y_data2, 'k-', linewidth=5, clip_on=False, zorder=15)  # linewidth=7 -> 10으로 증가
