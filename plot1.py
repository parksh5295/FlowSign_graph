...
def main():
    ...
    ax_f1.set_title("F1-score (%)", fontsize=80)
    ax_f1.set_xticks(list(x))
    ax_f1.set_xticklabels(datasets, fontsize=68, rotation=0, ha='center')
    ax_f1.tick_params(axis='x', pad=1)  # x축 레이블을 아래로 내림
    # y축 제목 제거
    # y축을 100 넘어가게 설정하되 y축 틱은 100까지만 표시
    ax_f1.set_ylim(0, 110)  # 100 넘어가게 설정
    ax_f1.tick_params(axis='y', labelsize=74)
    ...
    ax_acc.set_title("Accuracy (%)", fontsize=80)
    ax_acc.set_xticks(list(x))
    ax_acc.set_xticklabels(datasets, fontsize=68, rotation=0, ha='center')
    ax_acc.tick_params(axis='x', pad=1)  # x축 레이블을 아래로 내림
    # y축을 100 넘어가게 설정하되 y축 틱은 100까지만 표시
    ax_acc.set_ylim(0, 110)  # 100 넘어가게 설정
    ax_acc.tick_params(axis='y', labelsize=74)
    # y축 틱을 0-100까지만 표시 (100 넘어가는 값은 표기하지 않음)
